...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert abs(sigmoid(-a) - (1 - sigmoid(a))) < 1e-5
...
def test_symmetric() -> None:
    ...
    """
    assert mul(2.35, 3.42) == mul(3.42, 2.35)
...
def test_distribute() -> None:
    ...
    """
    assert mul(1.12, add(2.35, 3.42)) == add(mul(1.12, 2.35), mul(1.12, 3.42))
...
def test_other() -> None:
    ...
    """
    assert neg(neg(1.1)) == 1.1
    assert relu(1.1) == relu(relu(1.1))
    assert relu(-1.1) == relu(relu(-1.1))
